...
router.post("/register", limiter, async (req, res, next) => {
  try {
    ...
    const user = await db.one(
      `INSERT INTO users (email, password_hash, username, full_name, verify_token, verify_expires)
       VALUES ($1,$2,$3,$4,$5,$6) RETURNING id, email, username, role`,
      [rawEmail.toLowerCase(), hash, username, fullName || username, token, expires]
    );

    try { await email.sendVerificationEmail(rawEmail, token); } catch (e) {
...
router.post("/login", limiter, async (req, res, next) => {
  try {
    ...
    const user = await db.one("SELECT * FROM users WHERE email=$1", [rawEmail.toLowerCase()]);
    ...
    res.json({
      ...
      user: {
        ...
        levelType:   fresh.level_type,
        careerGoal:  fresh.career_goal,
        school:      fresh.school,
        xp:          fresh.xp,
        level:       fresh.level,
        streak:      fresh.streak,
        pomodoroMins: fresh.pomodoro_mins,
        pomodoroOn:   fresh.pomodoro_on,
      },
    });
  } catch (err) { next(err); }
});
...
router.post("/forgot-password", limiter, async (req, res, next) => {
  try {
    ...
    if (user) {
      ...
      try { await email.sendPasswordResetEmail(req.body.email, token); } catch (e) {
...
router.post("/onboarding", authenticate, async (req, res, next) => {
  try {
    ...
    await db.query(
      "UPDATE users SET level_type=$1, career_goal=$2 WHERE id=$3",
...
router.get("/me", authenticate, async (req, res, next) => {
  try {
    const user = await db.one(
      `SELECT id, email, username, full_name, avatar_url, bio, role, level_type,
              career_goal, school, is_public, is_verified, xp, level, streak,
              longest_streak, pomodoro_mins, pomodoro_on, created_at
...
async function updateStreak(userId) {
  ...
  if (lastActive === yesterday) {
    streak += 1; graceUsed = false;
  } else if (lastActive === dayBefore && !graceUsed) {
    streak += 1; graceUsed = true;
  } else {
    streak = 1; graceUsed = false;
  }

  await db.query(
    `UPDATE users SET streak=$1, last_active=$2, grace_used=$3,
     longest_streak=GREATEST(longest_streak,$1) WHERE id=$4`,
