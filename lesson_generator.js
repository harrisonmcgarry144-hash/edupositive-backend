...
["The Polarity of Water","Hydrogen Bonding in Water","Water as a Solvent","The Thermal Properties of Water","Water's Role in Living Organisms","Summary: Why Water is Essential for Life"]

Return ONLY the JSON array, no other text. Make each title specific and narrow.`;

  const text = await callAIWithRetry(SYSTEM_PROMPT, prompt, 500);
  try {
...
Each section: 1-2 SHORT paragraphs (2-4 sentences each). Total around 5 paragraphs max. Specific, concrete, exam-focused.`;

  return await callAIWithRetry(SYSTEM_PROMPT, prompt, 1500);
}
...
async function generateLessonsForSubtopic(subtopicId, examBoard, onProgress) {
  ...
  try {
    ...
    console.error(`Failed subtopic ${sub.subtopic}:`, e.message);
...
async function generateLessonsForSubject(subjectId, examBoard, onProgress) {
  const subtopics = await db.many(
    `SELECT st.id FROM subtopics st JOIN topics t ON t.id = st.topic_id
     WHERE t.subject_id = $1 ORDER BY t.order_index, st.order_index`,
    [subjectId]
  );
  let completed = 0;
...
async function getGenerationProgress(subjectId, examBoard) {
  const result = await db.one(
    `SELECT COUNT(DISTINCT st.id)::int AS total_subtopics,
     ...
     FROM subtopics st JOIN topics t ON t.id = st.topic_id WHERE t.subject_id = $1`,
    [subjectId, examBoard]
  );
